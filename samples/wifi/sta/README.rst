...
IP addressing
=============
The sample uses DHCP to obtain an IP address for the Wi-Fi interface, but it starts with a default static IP address
to handle networks without DHCP servers, or if the DHCP server is not available. Successful DHCP handshake will override
the default static IP configuration.

The default static configuration is below and can be changed in the ``prj.conf`` file:
