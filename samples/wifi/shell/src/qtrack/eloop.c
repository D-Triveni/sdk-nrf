...
static void qt_eloop_handle_signal(int sig)
{
	int i;
#if 0
#ifndef CONFIG_NATIVE_WINDOWS
...
static void qt_eloop_process_pending_signals(void)
{
	int i;
			  ...
			  void *user_data)
{
	...
	eloop.signal_count++;
	eloop.signals = tmp;
	signal(sig, qt_eloop_handle_signal);
...
void qt_eloop_run(void)
{
	...
	while (!eloop.terminate &&
		...
		qt_eloop_process_pending_signals();
