...
The sample provides the option to transmit raw TX packets either continuously or in a fixed number of packets.
...
* ``Channel``: Specifies the Wi-Fi channel to be used for communication on the wireless network.
...
* ``Inter-frame delay``: Sets the time duration between raw TX packets.
