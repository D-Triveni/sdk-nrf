...
void start_dhcp_server(char *if_name, char *ip_addr)
{
	...
	if (net_addr_pton(AF_INET, CONFIG_WFA_QT_SAMPLE_DHCPV4_POOL_START, &pool_start.s_addr)) {
		indigo_logger(LOG_LEVEL_ERROR, "Invalid address: %s", CONFIG_WFA_QT_SAMPLE_DHCPV4_POOL_START);
		return;
	}
	...
	if (ret == -EALREADY) {
		...
		indigo_logger(LOG_LEVEL_ERROR, "DHCPv4 server failed to start and returned %d error", ret);
	} else {
		indigo_logger(LOG_LEVEL_ERROR, "DHCPv4 server started and pool address starts from %s",
